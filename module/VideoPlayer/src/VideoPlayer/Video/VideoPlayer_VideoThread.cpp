...
//    int last_serial = -1;
    ...
    //OUTPUT("%s %f %f \n", __FUNCTION__, video_pts, audio_pts);
                ...
                // 丢帧逻辑：如果视频帧落后音频帧超过100ms，则丢帧
                if (video_pts < audio_pts - 0.04)  // 阈值设为40ms
                {
                    qDebug() << "Dropping video frame. video_pts:" << video_pts << " audio_pts:" << audio_pts;
                    av_packet_unref(packet);  // 释放当前帧的packet
                    goto drop_frame;  // 使用 goto 跳过后续处理，丢弃当前帧，解码下一帧
                }
...
drop_frame:  // 如果丢帧，跳转到这里，进入下一帧解码
            continue;  // 跳到下一个avcodec_receive_frame解码
        }
        av_packet_unref(packet);
    }
